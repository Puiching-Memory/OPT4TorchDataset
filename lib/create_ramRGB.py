...
def create_random_rgb_dataset(
    ...
    for i in track(range(num_images), description="Generating images"):
        image = np.random.randint(
            0, 256, size=(image_size[0], image_size[1], 3), dtype=np.uint8
        )
    ...
    logger.info(
        f"Dataset created: {num_images} images, {num_classes} classes at {output_dir}"
    )
